...
@patch("nemo_skills.pipeline.utils.generation.get_unmounted_path", lambda config, path: path)
def test_get_remaining_jobs_rerun_soft_failed_errors_relaunches_done_jobs():
    """Test that rerun_soft_failed_errors relaunches completed jobs and leaves row filtering to generation runtime."""
    with tempfile.TemporaryDirectory() as tmpdir:
        cluster_config = {"executor": "local", "mounts": []}
        random_seeds = [0, 1]
        chunk_ids = [None]

        create_done_files(tmpdir, [(0, None), (1, None)])
