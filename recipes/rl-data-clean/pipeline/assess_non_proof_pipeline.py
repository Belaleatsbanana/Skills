...
"""
Non-Proof Problem Assessment Pipeline

This pipeline assesses non-proof problems (natural language math problems)
with solutions for RL training quality.
"""
...
def assess_complete_solution_quality(cluster, expname, run_after, stage_config, **kwargs):
    ...
    generate(
        ctx=wrap_arguments(
            f"++prompt_config=/nemo-run/code/recipes/rl-data-clean/prompts/assess-complete-solution-quality.yaml "
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Non-Proof Problem Assessment Pipeline")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
