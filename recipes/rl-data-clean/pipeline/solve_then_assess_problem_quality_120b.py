...
"""
Solve-then-assess problem quality pipeline (gpt-oss-120b, proof).

Two stages only:
  1. solve_proof_for_quality: model attempts to solve each problem → output.jsonl
  2. assess_problem_quality_with_solution: judge (problem + solution) → accepted/rejected

Input: e.g. step-3 accepted.jsonl or a 5k sample. Output: accepted.jsonl / rejected.jsonl from stage 2.
"""
...
def solve_proof_for_quality(cluster, expname, run_after, stage_config, **kwargs):
    ...
    # Postprocess: copy generation → solution for stage 2
    postprocess_cmd = (
...
def assess_problem_quality_with_solution(cluster, expname, run_after, stage_config, **kwargs):
    """Stage 2: Judge problem quality using problem + solution. Output: accepted/rejected."""
    output_dir = stage_config["output_dir"]
    input_file = stage_config["input_file"]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Solve-then-assess problem quality (gpt-oss-120b, proof, 2 stages)")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
